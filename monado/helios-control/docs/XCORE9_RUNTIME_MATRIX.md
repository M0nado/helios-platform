...
3. Hybrid fleet: `pwsh -File monado/helios-control/scripts/Start-HeliosLocalFleet.ps1 -Mode Plan`, then `pwsh -Command "$p = Start-Process dotnet -ArgumentList @('run','--project','monado/helios-control/src/Helios.Connect.Api/Helios.Connect.Api.csproj','--configuration','Release','--','--urls','http://127.0.0.1:8080') -PassThru; Start-Sleep -Seconds 15; if ($p.HasExited) { throw 'Helios Connect API exited before hybrid health checks.' }"`, then `pwsh -File monado/helios-control/scripts/Start-HeliosLocalFleet.ps1 -Mode Status`, and `dotnet test monado/helios-control/Helios.Connect.sln --configuration Release --filter "FullyQualifiedName~XCore9GovernanceTests"`
