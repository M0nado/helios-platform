...
- Hardened Azure Bicep resources for managed identity, Key Vault, serverless
  Cosmos DB, Container Apps, Log Analytics, and Application Insights, with an
  external immutable ACR binding. Service Bus, ADLS, AI Search, and APIM remain
  architecture milestones rather than deployable connector resources.
...
- Edge one-button control runs with bounded per-replica queue hints, Cosmos ETag
...
- Online inbound webhook secret references, Service Bus fan-out, Foundry/Claude
  cloud adapters, APIM policies, and OpenTelemetry remain future slices. Cosmos
  now owns control-run durability and idempotency; Service Bus is still the
  scale-out queue milestone.
- Outbound relay code is implemented, but production bindings remain disabled
  until an administrator injects destination relay URLs and HMAC secrets from
  Key Vault and promotes delivery mode from `dry-run` to `live`.
...
3. Service Bus scale-out and dead-letter handling around the Cosmos-backed run store.
