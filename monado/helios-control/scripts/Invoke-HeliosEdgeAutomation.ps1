...
$deploymentArguments += $script:ResolvedDeploymentParameters
$deploymentArguments += @('--mode', 'Incremental')
$deployment = Invoke-AzJson -Arguments $deploymentArguments -Operation 'Applying reviewed HELIOS edge deployment'
[pscustomobject]@{
    mode = 'apply'
