...
class AzureDeploymentContractTests(unittest.TestCase):
    ...
    def test_azure_infra_routes_prod_apply_through_protected_environment(self) -> None:
        ...
        self.assertIn("resolved_platform_address_space: ${{ steps.params.outputs.resolved_platform_address_space }}", workflow)
        self.assertIn("echo \"resolved_platform_address_space=${resolved_platform_address_space}\" >> \"${GITHUB_OUTPUT}\"", workflow)
        self.assertIn('platform_vnet_name="$(printf', workflow)
        self.assertIn('existing_platform_address_space="$(az network vnet show', workflow)
        self.assertIn("Resolved platform address space '", workflow)
        ...
        self.assertIn("container_registry_id must reference a Premium Azure Container Registry", workflow)
        self.assertIn("Validated production container_registry_id must reference a Premium Azure Container Registry", workflow)
    ...
    def test_network_modules_support_non_overlapping_environment_address_plans(self) -> None:
        ...
        self.assertIn("param location string", hub_governance_module)
        self.assertIn("param platformAddressSpace string", hub_governance_module)
        self.assertIn("var approvedNetworkRules = networkPathPolicy.egress.?approvedNetworkRules ?? {}", hub_governance_module)
        self.assertIn("ruleType: 'NetworkRule'", hub_governance_module)
        self.assertIn("Allow-containerAppsPlatform-network-", hub_governance_module)
        self.assertIn("destinationAddresses: map(rule.destinationAddresses, destination => replace(destination, '{location}', location))", hub_governance_module)
    ...
    def test_network_path_catalog_includes_cosmos_firewall_destination(self) -> None:
        ...
        self.assertIn("*.documents.azure.com", network_paths)
        self.assertIn("*.blob.core.windows.net", network_paths)
        self.assertIn("containerAppsPlatform", network_paths)
        self.assertIn("packages.aks.azure.com", network_paths)
        self.assertIn("approvedNetworkRules", network_paths)
        self.assertIn("AzureCloud.{location}", network_paths)
        self.assertIn('"destinationPorts": ["1194"]', network_paths)
        self.assertIn('"destinationPorts": ["9000"]', network_paths)
