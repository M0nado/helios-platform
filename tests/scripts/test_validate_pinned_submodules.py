    ...
    def test_nested_dirty_worktree_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result, output = self._run_validator(
                root,
                {"approved": True, "submodules": [entry]},
                git_outputs=self._git_outputs(root, entry, nested_status_output=" M nested-change\n"),
            )

        self.assertEqual(1, result)
        self.assertIn("modules/example: nested submodule worktree is dirty", output)
