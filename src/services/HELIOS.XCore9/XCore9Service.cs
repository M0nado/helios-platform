...
{
    private static readonly HashSet<string> AllowedFeatures = new(StringComparer.Ordinal) { "success_rate", "duration_ratio", "cost_ratio", "tool_reliability", "route_accuracy", "retry_rate", "dependency_health", "holdout_score" };
    private readonly IXCoreAnalytics _analytics; private readonly IXCoreAuthorization _authorization; private readonly IXCoreAuditSink _audit; private readonly XCore9Options _options;
    private readonly IReadOnlyDictionary<string, WorkerTemplate> _templates; private readonly IReadOnlyDictionary<string, ToolchainDefinition> _toolchains; private readonly IReadOnlyDictionary<string, ToolDefinition> _tools;
    private readonly ConcurrentDictionary<Guid, WorkerLease> _leases = new(); private readonly List<RunHistoryEntry> _history = []; private readonly List<NegotiationRecord> _negotiations = []; private readonly object _gate = new(); private RoutingPolicy _activePolicy; private RoutingPolicy? _rollbackPolicy;
    ...
    public IReadOnlyList<RouteScore> ScoreRoutes(IReadOnlyList<CandidateRoute> candidates)
    {
        ...
        {
            if(!_templates.ContainsKey(candidate.WorkerTemplateId)||!_toolchains.ContainsKey(candidate.ToolchainId))
                throw new InvalidOperationException("Candidate references a non-approved template or toolchain.");
            if(candidate.Features.Any(x=>!AllowedFeatures.Contains(x.Name)))
                throw new InvalidOperationException("Candidate contains an unapproved feature.");
            if(candidate.Features.Select(x=>x.Name).Distinct(StringComparer.Ordinal).Count()!=candidate.Features.Count)
    ...
    public async ValueTask<PolicyDecision> EvaluatePromotionAsync(PromotionRequest request,string actor,CancellationToken token) { token.ThrowIfCancellationRequested(); await Demand(actor,"policy.promote",token); if(actor.StartsWith("xcore",StringComparison.OrdinalIgnoreCase)||request.RequestedBy.StartsWith("xcore",StringComparison.OrdinalIgnoreCase)) return new(false,"self-promotion-prohibited",_activePolicy,_rollbackPolicy); if(request.Holdout.SampleCount<_options.MinimumHoldoutSamples||!request.Holdout.Passed||request.Holdout.BaselineLoss-request.Holdout.CandidateLoss<_options.MinimumImprovement) return new(false,"holdout-requirement-not-met",_activePolicy,_rollbackPolicy); if(request.EvidenceLinks.Count==0) return new(false,"promotion-evidence-required",_activePolicy,_rollbackPolicy); lock(_gate) { _rollbackPolicy=_activePolicy; _activePolicy=request.Candidate with { PreviousPolicyId=_rollbackPolicy.PolicyId }; } await _audit.WriteAsync("policy-"+request.Candidate.PolicyId,"xcore9.policy.promoted",actor,request.EvidenceLinks,token); return new(true,"approved-by-external-authority",_activePolicy,_rollbackPolicy); }
