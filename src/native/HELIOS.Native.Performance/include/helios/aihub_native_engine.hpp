...
struct TriEngineNativeSignals {
  double csharpSafetyScore{};
  double fsharpLearningScore{};
  double cppNativeScore{};
  double vectorPressure{};
  double memoryPressure{};
  double integrationRisk{};
};

constexpr double tri_engine_native_fusion_score(const TriEngineNativeSignals& signals) noexcept {
  return clamp01((signals.csharpSafetyScore * 0.22) +
                 (signals.fsharpLearningScore * 0.24) +
                 (signals.cppNativeScore * 0.24) +
                 (signals.vectorPressure * 0.14) +
                 (signals.memoryPressure * 0.12) -
                 (signals.integrationRisk * 0.16));
}

constexpr bool should_parallel_optimize_all_three(const TriEngineNativeSignals& signals) noexcept {
  return tri_engine_native_fusion_score(signals) >= 0.66 && signals.csharpSafetyScore >= 0.50;
}

constexpr std::string_view tri_engine_native_label(const TriEngineNativeSignals& signals) noexcept {
  const auto score = tri_engine_native_fusion_score(signals);
  if (score >= 0.86) {
    return "parallel-csharp-fsharp-cpp-upgrade";
  }
  if (score >= 0.66) {
    return "integrated-three-engine-optimization";
  }
  return "stage-engines-sequentially";
}

}  // namespace helios::aihub
namespace helios::aihub {

struct FourEngineNativeSignals {
  double csharpSafetyScore{};
  double fsharpLearningScore{};
  double cppNativeScore{};
...
constexpr double four_engine_native_fusion_score(const FourEngineNativeSignals& signals) noexcept {
  return clamp01((signals.csharpSafetyScore * 0.20) +
                 (signals.fsharpLearningScore * 0.22) +
                 (signals.cppNativeScore * 0.22) +
                 (signals.pythonGlueScore * 0.18) +
                 (signals.vectorPressure * 0.12) -
                 (signals.integrationRisk * 0.16));
}

constexpr bool should_integrate_four_engine_parallel(const FourEngineNativeSignals& signals) noexcept {
  return four_engine_native_fusion_score(signals) >= 0.68 &&
         signals.csharpSafetyScore >= 0.55 &&
         signals.pythonGlueScore >= 0.45;
}

constexpr std::string_view four_engine_native_label(const FourEngineNativeSignals& signals) noexcept {
  const auto score = four_engine_native_fusion_score(signals);
  if (score >= 0.88) {
    return "parallel-csharp-fsharp-cpp-python-upgrade";
  }
  if (score >= 0.68) {
    return "integrated-four-engine-optimization";
  }
  return "stage-four-engines-with-csharp-first";
}
