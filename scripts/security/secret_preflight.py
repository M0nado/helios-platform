...
def main():
    ...
    for path in tracked_files():
        if not path.exists() or not should_scan(path): continue
        try: lines=path.read_text(errors='ignore').splitlines()
        except OSError: continue
        for no,line in enumerate(lines,1):
            if 'example' in path.name.lower() or 'template' in path.name.lower():
                continue
            placeholder=any(token in line.lower() for token in ['<', '>', 'your_', 'example', 'placeholder', 'xxx', '***', '$env:', 'getenvironmentvariable'])
            if placeholder:
                continue
