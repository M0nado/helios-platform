...
function Configure-Weights {
    ...
    $codex = Read-Host "  gpt-5.4-mini (default: 0.8)"
    $gpt45 = Read-Host "  gpt-5.5 (default: 0.9)"
    ...
    Write-Host "  gpt-5.4-mini: $codex"
    Write-Host "  gpt-5.5: $gpt45"

    return @{
        TaskType = $taskType
        Weights = @{
            ChatGPTPro = [double]$gpt4
            gpt_5_4_mini = [double]$codex
            GPT45 = [double]$gpt45
        }
    }
}
