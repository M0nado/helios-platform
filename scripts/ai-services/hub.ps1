...
.DESCRIPTION
Coordinates multiple OpenAI services (gpt-5.4, gpt-5.4-mini, gpt-5.5) with intelligent
...
class AIServiceHub {
    ...
    [void]ValidateConfiguration() {
        $registry = $this.Config.model_registry
        if (-not $registry -or $registry.validation_mode -ne "fail_closed") {
            throw "AI model registry is missing or not configured for fail_closed validation"
        }

        foreach ($serviceName in $this.Config.services.Keys) {
            $service = $this.Config.services[$serviceName]
            $model = [string]$service.model
            if ([string]::IsNullOrWhiteSpace($model)) {
                throw "Service '$serviceName' has an empty model id"
            }
            if ($registry.deprecated_model_ids -contains $model) {
                throw "Service '$serviceName' uses deprecated model id '$model'"
            }
            foreach ($pattern in $registry.placeholder_patterns) {
                if ($model -match $pattern) {
                    throw "Service '$serviceName' uses placeholder or misleading model id '$model'"
                }
            }
            if (-not ($registry.allowed_model_ids -contains $model)) {
                throw "Service '$serviceName' uses unknown model id '$model'"
            }
            if ($service.name -ne $model) {
                throw "Service '$serviceName' display name '$($service.name)' must match model id '$model'"
            }
        }
    }
    ...
    [hashtable]InitializeRateLimiter() {
        ...
        return @{
            RequestsPerMinute = $rateLimitConfig.global_per_minute
            RequestsPerHour = 0
            RequestsPerDay = $rateLimitConfig.global_per_day
            MinuteCounter = @{}
            HourCounter = @{}
            DayCounter = @{}
            LastReset = @{
    ...
    [bool]CheckRateLimit() {
        ...
        if ($this.RateLimiter.MinuteCounter[$minuteKey] -ge $rateLimitConfig.global_per_minute) {
    ...
    [array]DetermineServices([string]$TaskType) {
        ...
        $route = $this.Config.service_router.task_routing[$TaskType]
        if (-not $route) {
            $this.LogWarning("Unknown task type: $TaskType. Failing closed with no route.")
            return @()
        }
        foreach ($name in @($route.primary, $route.secondary)) {
            if ($name -and $this.Services.ContainsKey($name) -and $this.Services[$name].IsHealthy) {
                $servicesToUse += $this.Services[$name]
            }
            elseif ($name) {
                $this.LogWarning("Configured service unavailable or unhealthy: $name")
            }
        }
        return $servicesToUse
    }
    ...
    [PSCustomObject]CallServiceWithFallback([string]$ServiceName, [string]$TaskType, [string]$Content, [hashtable]$Options) {
        $route = $this.Config.service_router.task_routing[$TaskType]
        $fallbackOrder = @($route.primary, $route.secondary) | Where-Object { $_ -and $_ -ne $ServiceName }

        foreach ($fallbackService in $fallbackOrder) {
            if ($this.Services.ContainsKey($fallbackService) -and $this.Services[$fallbackService].IsHealthy) {
    ...
    [PSCustomObject]CombineResults([array]$Results, [string]$TaskType, [hashtable]$Options) {
        ...
        if ($this.Config.conflict_resolution.enabled) {
    ...
    [array]DetectConflicts([array]$Results) {
        ...
        for ($i = 0; $i -lt $Results.Count; $i++) {
            for ($j = $i + 1; $j -lt $Results.Count; $j++) {
                ...
                if ($similarity -lt (1 - ($this.Config.conflict_resolution.min_agreement_percent / 100))) {
                    $conflicts += @{
    ...
    [double]EstimateCost([string]$Content, [array]$Services) {
        ...
        foreach ($service in $Services) {
            $costPerToken = ($service.Config.cost_per_1k_tokens_input ?? $service.Config.cost_per_1k_tokens ?? 0) / 1000
    ...
    [bool]CanAffordRequest([double]$EstimatedCost) {
        if (-not $this.Config.cost_management.tracking_enabled) {
            return $true
        }

        $totalSpentToday = ($this.UsageTracker.Values | Measure-Object -Property CostAccumulated -Sum).Sum
        $dailyBudget = ($this.Config.cost_management.cost_limits.total_monthly_usd / 30)
...
function Invoke-AIServiceCall {
    ...
    $costPerToken = ($Config.cost_per_1k_tokens_input ?? $Config.cost_per_1k_tokens ?? 0) / 1000
