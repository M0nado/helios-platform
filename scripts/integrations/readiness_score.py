...
def node_ok(tag):
    ...
    return bool(tagged) and all(r.get('status') in {'passed','skipped/dry-run'} for r in tagged)
categories={
 ...
 'azure':[bool_check('azure-cli','az',shutil.which('az')), bool_check('azure-report','azure status','azure' in control)],
...
finish={'generatedUtc':payload['generatedUtc'],'score':finish_score,'categoryScores':category_scores,'categories':categories,'blockers':blockers,'buildGraphCounts':build.get('counts',{}),'nextFixes':build.get('nextFixes',[])[:10]}
