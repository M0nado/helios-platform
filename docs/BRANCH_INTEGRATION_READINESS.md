...
2. Configure remotes automatically: `./helios.sh specialist-check --auto-setup-remote --remote-url <url>`, or omit `--remote-url` to infer it from `HELIOS_REMOTE_URL`, `REMOTE_URL`, `GITHUB_REPOSITORY`, `git config github.repository`, GitHub CLI, or historical GitHub merge subjects; then fetch all branch refs with `git fetch --all --prune`.
...
The checker is intentionally safe by default: it reports missing prerequisites and only runs Azure login, Azure default configuration, remote setup, or push when explicit switches are provided. For CI or Codespaces, set `HELIOS_REMOTE_URL` or rely on automatic inference, then run `./helios.sh specialist-check --auto-setup-remote --push-current`. A PowerShell wrapper with matching intent is also available at `scripts/setup/setup-specialist-environment.ps1` for Windows-oriented operators.
